        ...
        while (list($key, $value) = each($Result)) {
            if ($DiscoveryPatternSet != "") {
                ...
                if ($Match) {
                    ...
                    $ParsedAbout = substr(strip_tags(str_replace(array("\n","\r", "|"), '',$value["About"])),0,50) . "...";
                    $ParsedDiscoveryText = substr(strip_tags(str_replace(array("\n","\r", "|"), '',$value["DiscoveryText"])),0,50) . "...";


                    $val = $value["UserID"] . "|" . $value["Name"] . "|" . $Rolenames . "|" . $value["Email"] . "|" . $value["UpdateIPAddress"] . "|" . Gdn_Format::Date($value["DateLastActive"]) . "|" . $value["CountDiscussions"] . "|" . $value["CountComments"] . "|" . $ParsedAbout . "|" . $ParsedDiscoveryText . PHP_EOL;
                    fwrite($fp, $val);
                }
            } else {
                $UID = $value["UserID"];
